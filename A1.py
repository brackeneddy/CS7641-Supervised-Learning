...
car_evaluation = pd.concat([car_evaluation_data.data.features, car_evaluation_data.data.targets], axis=1)

# Preprocess wholesale
wholesale_customers['Total Spend'] = wholesale_customers[['Fresh', 'Milk', 'Grocery', 'Frozen', 'Detergents_Paper', 'Delicassen']].sum(axis=1)
wholesale_customers['Category'] = pd.qcut(wholesale_customers['Total Spend'], q=3, labels=['Low', 'Medium', 'High'])

label_encode_wholesale = LabelEncoder()
wholesale_customers['Category'] = label_encode_wholesale.fit_transform(wholesale_customers['Category'])
...
x_wholesale_train, x_wholesale_test, y_wholesale_train, y_wholesale_test = train_test_split(x_wholesale_scaled, y_wholesale, test_size=0.3, random_state=42)
...
X_car_train, X_car_test, y_car_train, y_car_test = train_test_split(X_car_scaled, y_car, test_size=0.3, random_state=42)
...
nn_grid_search = GridSearchCV(MLPClassifier(), param_grid_nn, cv=5, scoring='accuracy', n_jobs=-1)
...
svm_grid_search = GridSearchCV(SVC(), param_grid_svm, cv=5, scoring='accuracy', n_jobs=-1)
...
knn_grid_search = GridSearchCV(KNeighborsClassifier(), param_grid_knn, cv=5, scoring='accuracy', n_jobs=-1)
...
ada_grid_search = GridSearchCV(AdaBoostClassifier(estimator=ada_estimator), param_grid_ada, cv=5, scoring='accuracy', n_jobs=-1)
...
plot_learning_curve(best_nn_wholesale, "Learning Curve (Best NN, Wholesale Customers)", x_wholesale_train, y_wholesale_train)
plot_learning_curve(best_svm_wholesale, "Learning Curve (Best SVM, Wholesale Customers)", x_wholesale_train, y_wholesale_train)
plot_learning_curve(best_knn_wholesale, "Learning Curve (Best k-NN, Wholesale Customers)", x_wholesale_train, y_wholesale_train)
plot_learning_curve(best_ada_wholesale, "Learning Curve (Best AdaBoost, Wholesale Customers)", x_wholesale_train, y_wholesale_train)
...
plot_learning_curve(best_nn_car, "Learning Curve (Best NN, Car Evaluation)", X_car_train, y_car_train)
plot_learning_curve(best_svm_car, "Learning Curve (Best SVM, Car Evaluation)", X_car_train, y_car_train)
plot_learning_curve(best_knn_car, "Learning Curve (Best k-NN, Car Evaluation)", X_car_train, y_car_train)
plot_learning_curve(best_ada_car, "Learning Curve (Best AdaBoost, Car Evaluation)", X_car_train, y_car_train)
